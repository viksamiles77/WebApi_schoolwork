        ...
        public ActionResult<Note> GetByIndex(int index)
        {
            try
            {
                ...
                if (index > StaticDb.Notes.Count)
                    return BadRequest($"Cannot find resource on index {index}");
        ...
        public ActionResult<List<Note>> FilterNotesByMultipleParams(string? text, int? priority)
        {
            try
            {
                if (string.IsNullOrEmpty(text) && priority == null)
                    return BadRequest("You have to sendat least one parameter!");
                ...
                List<Note> notes = StaticDb.Notes.Where(x => x.Text.ToLower().Contains(text.ToLower()) && priority == (int)x.Priority).ToList();
                return Ok(notes);
