...
namespace Qinshift.EShop.DataAccess.DapperImplementation
{
    public class CategoryDapperRepository : IRepository<Category>
    {
        ...
        public Category GetById(int id)
        {
            ...
            {
                connection.Open();
                //Category category = connection.QuerySingleOrDefault<Category>("SELECT * FROM dbo.Categories", new { Id = id });
        ...
        public int Add(Category entity)
        {
            using (SqlConnection connection = new SqlConnection(_connString))
            {
                ...
                string query = "INSERT INTO dbo.Categories (Name, Description) VALUES (@name, @description)";
